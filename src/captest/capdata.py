...
class CapData(object):
    ...
    def expand_agg_map(self, agg_map):
        ...
        for key, value in agg_map.items():
            if isinstance(value, dict):
                ...
                agg_columns = [f"{sub_key}_{sub_value}_agg" 
                             for sub_key, sub_value in value.items()]
                self.column_groups[aggs_key] = agg_columns
                
                # Add the aggs key to the expanded map
                expanded_map[aggs_key] = value[next(iter(value))]  # Use same agg function as subgroups
                rename_map[f"{aggs_key}_mean_agg"] = f"{key}_{value[next(iter(value))]}_agg"
    ...
    def filter_power(self, power, percent=None, columns=None, inplace=True):
        ...
        if multiple_columns:
            ...
            #                                        axis=1)
            filtered_power_bool = power_data.apply(lambda x: all(x.le(power, fill_value=True)),
                                                   axis=1)
        else:
